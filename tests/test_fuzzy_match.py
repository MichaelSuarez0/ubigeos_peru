...
class TestFuzzyMatch:
    def test_fuzzy_match_departamento(self, fuzzy_match_test_cases):
        ...
        for original, expected in test_cases.items():
            result = ubg.validate_departamento(
                original, fuzzy_match=True, on_error="ignore"
            )

            assert result == expected, (
                f"Fallo en departamento: '{original}' → '{result}', esperado: '{expected}'"
            )
    ...
    def test_fuzzy_match_provincia(self, fuzzy_match_test_cases):
        ...
        for original, expected in test_cases.items():
            result = ubg.validate_provincia(
                original, fuzzy_match=True, on_error="ignore"
            )

            assert result == expected, (
                f"Fallo en provincia: '{original}' → '{result}', esperado: '{expected}'"
            )
    ...
    def test_fuzzy_match_distrito(self, fuzzy_match_test_cases):
        ...
        for original, expected in test_cases.items():
            result = ubg.validate_distrito(
                original, fuzzy_match=True, on_error="ignore"
            )

            assert result == expected, (
                f"Fallo en distrito: '{original}' → '{result}', esperado: '{expected}'"
            )
